...
EXTAPI_DLLEXPORT simxInt simxCustomSetJointsTargetPositions(simxInt clientId, simxInt handleCount, simxInt* handles, simxFloat* positions, simxInt operationMode)
{
  simxInt returnValue;
  /* First catch a possible error: */
  if (_communicationThreadRunning[clientId]==0)
  ...
  if (operationMode==simx_opmode_remove)
  {
    return(_removeCommandReply_null(clientId,simx_customcmd_set_joints_target_positions)); /* _int indicates that the command is identified by the command ID AND an integer value */
  }
  /* Prepare the command buffer */
  simxInt bufferSize = sizeof(simxInt) + handleCount*sizeof(simxInt) + handleCount*sizeof(simxFloat);
  simxUChar * buffer = (simxUChar*)calloc(bufferSize, 1);
  memcpy(buffer, &handleCount, sizeof(simxInt));
  memcpy(&(buffer[sizeof(simxInt)]), handles, handleCount*sizeof(simxInt));
  memcpy(&(buffer[sizeof(simxInt) + handleCount*sizeof(simxInt)]), positions, handleCount*sizeof(simxFloat));
  /* Execute the command */
  _exec_null_buffer(clientId,simx_customcmd_set_joints_target_positions,operationMode,0,buffer,bufferSize,&returnValue);
  /* return the error code */
  /* FIXME Who is taking care of the memory we allocated here? */
  return(returnValue);
}
